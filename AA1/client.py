...
def left_board():
    ...
    xPos = box_width
    # Create variable yPos

    # Change range to 0 to 15
    for box in range(0,10):
        ...
        # Change y position to yPos
        box_label.place(x=xPos, y=screen_height/3)
        left_boxes.append(box_label)
        # Increment yPos if box is less then 5
        
        # Otherwise increment Xpos
        xPos += box_width*2
...
def right_board():
    ...
    # Create yPos variable with value box_width
    
    # Change the range to 0 to 15
    for box in range(0,10):
        if(box == 0):
