...
def roll_dice():
    ...
    # Get a random choice in value2 variable

    player_turn = False
    roll_button.destroy()

    if(player_type == 'player1'):
        # Send dice1 in message
        SERVER.send(f'{value}player2_turn'.encode())
        # Create another message to send value2 and dice2 in message
...
def received_msg():
    global SERVER, canvas2, dice, player_turn, player_type, roll_button, dice2
       
    while True:
        message = SERVER.recv(2048).decode()
        
        # Check if 'dice1 in message
        
        # Indent this if elif block 
        if('\u2680' in message):
        ...
        if(('player1_turn' in message and player_type == 'player1') or
           ('player2_turn' in message and player_type == 'player2')
            ...
            roll_button.place(x=(screen_width * 0.5) - font_size*2, y= screen_height * 0.8)

def game():
    global game_window, canvas2, screen_width, screen_height, dice, wining_message, font_size, image, roll_button, dice2
    ...
    # Create another dice named dice2
